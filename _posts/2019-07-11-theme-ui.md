...
First I'm going to show you how you can utilize the theme object to style with Emotion. We can do this with the packages we have already installed. Then we'll refactor the code to take advantage of some of the nice features of Theme UI. Below is the `<Card />` component with a couple of elements styled using the `css` prop from Emotion:
...
It does what we want it to do. If you've never used Emotion's `css` prop the first two lines might look a little strange to you. This is required in order to use the `css` prop. It essentially informs Babel to use Emotion’s jsx function instead of `React.createElement` when compiling the jsx in this file.
